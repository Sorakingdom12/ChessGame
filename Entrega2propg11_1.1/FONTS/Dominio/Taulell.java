...
//funciones
            ...
            if ( !fin_fichas ) {
                ...
                if (Fen_act.charAt(k) != '/') {
                    ...
                    if (Fen_act.charAt(k) == 'r') {
                        Negra.add(new Torre(auxn, "torre", "negra", 'r', i, j, 5.5));
                        Tablero[i][j] = auxn;
                        ++j;
                        auxn++;
                    }
                    ...
                    /* lectura de una rey negro*/
                        ...
                        Negra.add(new Rey(auxn, "rey", "negra",'k' , i, j, 10000));
                    ...
                    /* lectura de una reina negra*/
                        ...
                        Negra.add(new Reina(auxn, "reina", "negra", 'q', i, j, 10));
                        Tablero[i][j] = auxn;
                        ++j;
                        auxn++;
                    }
                    ...
                    /* lectura de una alfil negro*/
                        ...
                        Negra.add(new Alfil(auxn, "alfil", "negra", 'b' , i, j, 3.5));
                        Tablero[i][j] = auxn;
                        ++j;
                        auxn++;
                    }
                    ...
                    /* lectura de una caballo negro*/
                        ...
                        Negra.add(new Caball(auxn, "caball", "negra",'n' , i, j, 3));
                        Tablero[i][j] = auxn;
                        ++j;
                        auxn++;
                    }
                    ...
                    /* lectura de una peon negro*/
                        ...
                        Negra.add(new Peo(auxn, "peo", "negra",'p' , i, j, 1));
    ...
    boolean dentro_del_tablero(int x, int y) {
    ...
    Ficha promover_peon(Ficha f, String forma) {
        int posx = f.get_pos_x();
        int posy = f.get_pos_y();
        if (f.get_color().equals("blanca")) {
            ...
            if (forma.equals("reina")) {
                //Blanca.remove(index);
                Blanca.set(index, new Reina (index+16, "reina", "blanca",'Q', posx, posy, 10.0));
                String nom = get_ficha(index+16).get_nom();
            }
            else if (forma.equals("caball")) {
                //Blanca.remove(index);
                Blanca.set(index, new Caball (index+16, "caball", "blanca",'N', posx, posy, 3.0));
            }
            else if (forma.equals("torre")) {
                //Blanca.remove(index);
                Blanca.set(index, new Torre (index+16, "torre", "blanca",'R', posx, posy, 5.5));
            }
            else if(forma.equals("alfil")) {
                //Blanca.remove(index);
                Blanca.set(index, new Alfil (index+16, "alfil", "blanca",'B', posx, posy, 3.5));
            }
        }
            ...
            if (forma.equals("reina")) {
                //Negra.remove(index);
                Negra.set(index, new Reina (index, "reina", "negra",'r', posx, posy, 10.0));
            }
            if (forma.equals("caball")) {
                //Negra.remove(index);
                Negra.set(index, new Caball (index, "caball", "negra",'n', posx, posy, 3.0));
            }
            if (forma.equals("torre")) {
                //Negra.remove(index);
                Negra.set(index, new Torre (index, "torre", "negra",'r', posx, posy, 5.5));
            }
            if (forma.equals("alfil")) {
                //Negra.remove(index);
                Negra.set(index, new Alfil (index, "alfil", "negra",'b', posx, posy, 3.5));
            }
        }
        return f;
    }
    ...
    void despromover(Ficha f, Ficha origen) {
        //int posx = f.get_pos_x();
        //int posy = f.get_pos_y();
        if (f.get_color().equals("blanca")) {
            int index = f.get_id()-16;
    ...
    boolean camino_ok(int pos_x, int pos_y, int mov_x, int mov_y, int count, String colorf) {
        int x = pos_x, y = pos_y;
            ...
            return false;
                ...
                if (get_nom_ficha(Tablero[pos_x][pos_y]).equals("rey")) {
                    ...
                    if (Tablero[pos_x][pos_y] >= 0 && Tablero[pos_x][pos_y] <= 15) {
                        if (!Negra.get(Tablero[pos_x][pos_y]).pos_ok(x,y,this)) return false;

                    }
                    if (Tablero[pos_x][pos_y] >= 16 && Tablero[pos_x][pos_y] <= 31) {
                        if (!Blanca.get(Tablero[pos_x][pos_y] - 16 ).pos_ok(x,y,this)) return false;
                    }
                }
        }
    ...
    boolean jaque(String color) {
        ...
        while (dentro_del_tablero(x,y)) {
            if (Tablero[x][y] != -1) {
                if (get_nom_ficha(Tablero[x][y]).equals("reina") || get_nom_ficha(Tablero[x][y]).equals("torre")) {
                    if (color.equals("blanca") && get_color_ficha(Tablero[x][y]).equals("negra")) return true;
                    if (color.equals("negra") && get_color_ficha(Tablero[x][y]).equals("blanca")) return true;
                }
                break;
            }
            x--;
        ...
        x = posx + 1;
        y = posy + 1;
        if (dentro_del_tablero(x,y) && Tablero[x][y] != -1 && get_nom_ficha(Tablero[x][y]).equals("peo")) {
        ...
        while (dentro_del_tablero(x,y)) {
            if (Tablero[x][y] != -1) {
                if (get_nom_ficha(Tablero[x][y]).equals("reina") || get_nom_ficha(Tablero[x][y]).equals("alfil")) {
                    if (color.equals("blanca") && get_color_ficha(Tablero[x][y]).equals("negra")) return true;
                    if (color.equals("negra") && get_color_ficha(Tablero[x][y]).equals("blanca")) return true;
                }
                break;
        ...
        for (int i = 0; i < 8; i++) {
            x = posx + cax[i];
            y = posy + cay[i];
            if (dentro_del_tablero(x,y) && Tablero[x][y] != -1 && get_nom_ficha(Tablero[x][y]).equals("caball")) {
                if (color.equals("blanca") && get_color_ficha(Tablero[x][y]).equals("negra")) return true;
                if (color.equals("negra") && get_color_ficha(Tablero[x][y]).equals("blanca")) return true;
            }
        }
    ...
    ArrayList<Nodo> todos_movs(int color,double papipoints) {
    ...
    void puntua ( Nodo n, boolean OP, boolean promocion){
        // Analiza Play
     ...
     boolean amenaza(int posx, int posy, String color ) {
        ...
        y = posy + 1;
        if (dentro_del_tablero(x,y) && Tablero[x][y] != -1){
            if (get_nom_ficha(Tablero[x][y]).equals("peo")) {
                //Si el peón es blanco y el rey negro -> amenaza
                if (color.equals("negra") && get_color_ficha(Tablero[x][y]).equals("blanca")) return true;
                x = -1; //no entramos en el while
            }
    ...
    double ataca (Ficha f,String Color) {
        double puntos = 0.0;
    ...
    double Asegura (Ficha f,String Color) {
        double puntos = 0.0;
        String nom = f.get_nom();
    ...
    void puntua_tonto ( Nodo n, boolean OP, boolean promocion) {
        // Analiza Play
        String Color1 = n.get_ficha().get_color();
        String Color2;
        if (Color1.equals("blanca")) Color2 = "negra";
        ...
        boolean prioMenace = amenaza( auxX, auxY, Color1); // Estoy Expuesto?
        boolean prioProtec = amenaza( auxX, auxY, Color2); // Estoy Protegido?
        double  prioSecure = Asegura_tonto( n.get_ficha(),Color2); // PROTEGO?
        double  prioattaca = ataca_tonto( n.get_ficha(),Color1); // AMENAZO?
        //pintar_tablero();
        ...
        boolean postMenace = amenaza( n.get_posx(), n.get_posy(), Color1); // EXPOSED?
        boolean postProtec = amenaza( n.get_posx(), n.get_posy(), Color2); // PROTECTED?
        double  postSecure = Asegura_tonto(n.get_ficha(),Color2); // PROTEGO?
        double  postattaca = ataca_tonto(n.get_ficha(),Color1); // AMENAZO?
        ...
        if (!prioMenace && postMenace) Danger += 2*me;
        if (prioMenace && !postMenace) Danger -= me;
        if (!n.get_ficha().get_nom().equals("rey") && prio_check && !post_check) Danger -= 10;
        if (prioProtec && !postProtec && !n.get_ficha().get_nom().equals("rey")) Dprotc += me;
    ...
    private double Asegura_tonto (Ficha f,String Color) {
        double puntos = 0.0;
        String nom = f.get_nom();
    ...
    private double ataca_tonto (Ficha f,String Color) {
        double puntos = 0.0;
        String nom = f.get_nom();
        int posx, posy;
        int x = posx = f.get_pos_x();
        ...
        if (nom.equals("peo")) {
            if (Color.equals("blanca")) {
                ...
                if (dentro_del_tablero(x,y)) {
                    if (Tablero[x][y] != -1 && get_color_ficha(Tablero[x][y]).equals("negra")) {
                        if (get_nom_ficha(Tablero[x][y]).equals("rey")) puntos += 10.0;
                        else puntos += 1.0;
                    }
                    //puntos += get_ficha(Tablero[x][y]).getPuntuacionBase();
                }
